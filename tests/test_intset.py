...
# Valid IntSet elements, biased towards small values (so that independently
# generated sets are likely to overlap) and towards having each individual bit
# set (so that every level of the underlying trie gets exercised).
integers_in_range = st.one_of(
    st.integers(min_value=0, max_value=SMALL),
...
short_intervals = st.builds(
    lambda start, length: assume(start + length <= 2**64) and (start, start + length),
    integers_in_range,
    st.integers(0, SMALL),
...
intervals = (
    st.tuples(integers_in_range, integers_in_range).map(lambda x: sorted(tuple(x)))
    | short_intervals
)

interval_list = st.lists(intervals)

IntSets = st.one_of(
    st.builds(IntSet.from_intervals, interval_list),
    integers_in_range.map(IntSet.single),
    intervals.map(lambda x: IntSet.interval(*x)),
    st.builds(IntSet.from_iterable, st.lists(integers_in_range)),
)


SmallIntSets = st.builds(IntSet.from_intervals, st.lists(short_intervals)).filter(
    lambda x: x.size() <= SMALL
)


def intsets_of_size_at_least(n):
    return IntSets.filter(lambda x: x.size() >= n)


NonEmptyIntSets = intsets_of_size_at_least(1)
...
def test_intersect_alternating_elements_is_empty(imp):
    ...
    for i, x in enumerate(imp):
        if i % 2 == 0:
            evens = evens.discard(x)
        else:
            odds = odds.discard(x)
    assert not (evens & odds)
    assert evens.isdisjoint(odds)


@given(IntSets, IntSets, NonEmptyIntSets)
def test_removing_each_from_the_other_is_disjoint(x, y, common):
    x |= common
    y |= common
...
def test_iterable_equivalent_to_intervals_of_length_one(xs):
    assert IntSet.from_iterable(xs) == IntSet.from_intervals((x, x + 1) for x in xs)
...
@example(
    [[0, 1], [13356455824667928169, 13356455824667928173], [0, 13356455824667928169]]
)
@example(
    [
        [7341163011306295800, 7341163011306295801],
        [0, 7341163011306295799],
...
@example(
    [
        ...
        [5072311219282295775, 5072311219282295776],
        [0, 5072311219282295775],
    ]
)
@example([[2, 5], [0, 1]])
@example([[0, 1], [0, 1]])
...
@example(
    [
        [11545114544614188051, 11545114544614188052],
        [0, 11545114544614188049],
...
def test_concatenation_of_lists_is_union(x, y):
    assert IntSet.from_intervals(x + y) == IntSet.from_intervals(
        x
    ) | IntSet.from_intervals(y)
...
@given(intsets_of_size_at_least(2))
...
def internal_index(imp, i):
    """Map an arbitrary non-negative integer to an index into imp that is
    neither the first nor the last element."""
    return 1 + i % (imp.size() - 2)


@example(IntSet.interval(0, 3), 1)
@example(IntSet.interval(0, 4), 2)
@given(intsets_of_size_at_least(3), st.integers(min_value=0))
def test_deleting_an_internal_element_produces_a_larger_intset(imp, i):
    imper = imp.discard(imp[internal_index(imp, i)])
    assert_order(imp, imper)
...
def test_deleting_an_internal_element_produces_a_subset(imp, i):
    impy = imp.discard(imp[internal_index(imp, i)])
    assert_strict_subset(impy, imp)


@given(intsets_of_size_at_least(3))
...
@example(
    IntSet([(9223372036854770807, 9223372036854770864)]), IntSet([9223372036854770814])
)
@given(SmallIntSets, SmallIntSets)
...
@example(IntSet([0, (2016, 2032), (2032, 2048), 2048]), IntSet([2048]), IntSet([2050]))
...
@example(
    IntSet.from_intervals(
        [
            (0, 1),
            (9223372036854775791, 9223372036854775792),
...
@example(
    IntSet.from_intervals(
        [
            (0, 1),
            (9943224696285111261, 9943224696285111296),
...
class SetModel(RuleBasedStateMachine):
    ...
    @rule(target=intsets, x=intsets, ints=st.lists(integers_in_range, min_size=1))
...
@example([[2, 3], [0, 1], [0, 4]])
@example(
    [[2, 15738258725653508269], [0, 1], [15738258725653508269, 15738258725653508271]]
)
@given(interval_list)
...
def test_single_accepts_largest_valid_value():
    largest = 2**64 - 1
    x = IntSet.single(largest)
    assert list(x) == [largest]
    assert x == IntSet.empty().insert(largest)
    assert x == IntSet.interval(largest, 2**64)
    assert x == IntSet.from_iterable([largest])
